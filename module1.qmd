...
```{r}
pp_pipeline <- preProcess(iris[,1:4], method = c("center", "scale")) ## centering and scaling the dataset
iris_scaled <- predict(pp_pipeline, iris[,1:4]) ## applying the behaviour to the dataset
pca_res <- prcomp(iris_scaled) ## getting the princple components
# summary(pca_res)
...
# 2. Run PCA
pca1 <- prcomp(scaled_data, scale. = TRUE)
